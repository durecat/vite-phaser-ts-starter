...
const intro = {
  ...
  create: function() {
    ...
    this.add
			.text(x, y + 30, "Choose your hero", {
				font: "28px 'Doomed'",
				color: "#cfc6b8",
			}).setOrigin(0.5);
    
    // Display available heros
    let classNames = Object.keys(classes)
    for(let h = 0; h < classNames.length; h++) {
      let inc = 50 * h
      this.add
				.text(x, y + 80 + inc, `${h + 1} - ${classNames[h]}`, 
        {
					font: "24px 'Doomed'",
					color: "#cfc6b8",
				}).setOrigin(0.5);
    ...
    // Handling player's input and set the dungeon.hero
    this.input.keyboard.on("keyup", (event) => {
      ...
      if(!isNaN(Number(key))) {
        ...
        if(hero) {
          dungeon.hero = hero // set the hero the player has chosen
          this.scene.stop()
